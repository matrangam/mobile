...
import (
	...
	"github.com/StartisIOT/gomobile/bind"
...
func genPkg(lang string, p *types.Package, astFiles []*ast.File, allPkg []*types.Package, classes []*java.Class, otypes []*objc.Named) {
	...
	case "java":
		...
		if p == nil {
			p, err := build.Default.Import("github.com/StartisIOT/gomobile/bind", ".", build.ImportComment)
			if err != nil {
				errorf(`"github.com/StartisIOT/gomobile/bind" is not found; run go get github.com/StartisIOT/gomobile/bind: %v`, err)
				return
			}
			repo := filepath.Clean(filepath.Join(p.Dir, "..")) // github.com/StartisIOT/gomobile directory.
			...
			// Copy support files
			javaPkg, err := build.Default.Import("github.com/StartisIOT/gomobile/bind/java", "", build.FindOnly)
			if err != nil {
	...
	case "go":
		...
		io.Copy(w, &buf)
		closer()
		bindPkg, err := build.Default.Import("github.com/StartisIOT/gomobile/bind", "", build.FindOnly)
		if err != nil {
	...
	case "objc":
		...
		if p == nil {
			// Copy support files
			objcPkg, err := build.Default.Import("github.com/StartisIOT/gomobile/bind/objc", "", build.FindOnly)
			if err != nil {
